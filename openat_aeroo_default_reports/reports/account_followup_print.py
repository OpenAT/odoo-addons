...
class Parser(report_sxw.rml_parse):
    ...
    def _lines_get(self, stat_by_partner_line):
        res = self._lines_get_with_partner(stat_by_partner_line.partner_id, stat_by_partner_line.company_id.id)
        print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", res
        return res
    ...
    def _get_text(self, stat_line, followup_id, context=None):
        ...
        if text:
            try:
                ...
                text = 'ACHTUNG!!! FEHLER in de Mahnungs Text im TEMPLATE unter "Finanzen" "Konfiguration" "Mahnstufen" . Bitte Korrigieren sie den Text: INFO: https://lists.launchpad.net/openerp-india/msg11589.html \n%r' % text
